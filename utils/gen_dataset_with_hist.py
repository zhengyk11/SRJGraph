    ...
    for f in ['back.tsv', 'train.tsv', 'dev.tsv', 'test.tsv']:
        ...
        for arr in all_data:
            ...
            if int(click) == 1:
                ...
                uid_cate_hist[uid][item_cate_level1_id] = hist_cate[:max_hist_len+10]

        if len(data_type_list) == 1:
            out_dir = '../dataset/dataset_0929/{}_hist'.format(data_type_list[0])
        else:
            out_dir = '../dataset/dataset_0929/all_hist'

        if not os.path.exists(out_dir):
            os.mkdir(out_dir)
        output = open(out_dir + '/{}_hist.tsv'.format(f.replace('.tsv', '')), 'w')
