    ...
    def load_data(self, data_path):
        ...
        # for line in lines:
            ...
            if self.need_neighbor:
                line += '\t' + self.get_all_neighbors(user_id, 'user', item_cate_level1_id, pv_time, self.user_neighbor_nums)
                line += '\t' + self.get_all_neighbors(nid, 'item', item_cate_level1_id, pv_time, self.item_neighbor_nums)
...
if __name__ == '__main__':
    ...
    class config():
        def __init__(self):
            ...
            self.user_cate_item_table_path = '../dataset/dataset_0929/user_cate_item_table_{}.pkl'.format(data_type)
            self.item_user_table_path = '../dataset/dataset_0929/item_user_table_{}.pkl'.format(data_type)
    ...
    out_dir = '../dataset/dataset_0929/{}_cate_hist_graph_parts_{}_{}'.format(data_type, '-'.join(map(str, args.user_neighbor_nums)), '-'.join(map(str, args.item_neighbor_nums)))
    time.sleep(line_num)
    ...
    if line_num in range(line_split): # map(str, range(20200820, 20200832) + range(20200901, 20200913))
        ...
        output = open(out_dir + '/{}_cate_hist_graph_{}.tsv'.format(data_split, line_num), 'w')

        if data_split == 'train':
            d = Dataset(args,
                        train_files=['../dataset/dataset_0929/{}_cate_hist/train_cate_hist.tsv'.format(data_type)],
                        need_neighbor=True)
        elif data_split == 'dev':
            d = Dataset(args,
                        dev_files=['../dataset/dataset_0929/{}_cate_hist/dev_cate_hist.tsv'.format(data_type)],
                        need_neighbor=True)
        else:
            d = Dataset(args,
                        test_files=['../dataset/dataset_0929/{}_cate_hist/test_cate_hist.tsv'.format(data_type)],
                        need_neighbor=True)

        output.close()
    else:
        output = open(out_dir + '/{}_cate_hist_graph.tsv'.format(data_split), 'w')
        for root, dirs, files in os.walk(out_dir):
            files = filter(lambda x:x.endswith('.tsv') and x.startswith('{}_cate_hist_graph_'.format(data_split)), files)
