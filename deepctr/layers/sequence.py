...
from tensorflow.python.keras.layers import LSTM, Lambda, Layer, Embedding
...
class Transformer(Layer):
    ...
    def __init__(self, att_embedding_size=1, head_num=8, dropout_rate=0.0, use_positional_encoding=True, use_res=True,
                 use_feed_forward=True, use_layer_norm=True, blinding=False, seed=1024, supports_masking=False,
                 **kwargs):
        if head_num <= 0:
    ...
    def build(self, input_shape):
        ...
        self.seq_len_query = int(input_shape[0][-2])
        self.seq_len_key = int(input_shape[1][-2])
        assert max(self.seq_len_query, self.seq_len_key) <= self.seq_len_max
        self.W_Query = self.add_weight(name='query_weight', shape=(embedding_size, self.att_embedding_size * self.head_num),
                                       dtype=tf.float32,
        ...
        self.B_Key = self.add_weight(name='key_bias', shape=(self.att_embedding_size * self.head_num,),
                                       dtype=tf.float32,
                                       initializer=tf.keras.initializers.TruncatedNormal(seed=self.seed))
        self.W_Value = self.add_weight(name='value_weight', shape=(embedding_size, self.att_embedding_size * self.head_num),
                                       dtype=tf.float32,
                                       initializer=tf.keras.initializers.TruncatedNormal(seed=self.seed + 2))
        self.B_Value = self.add_weight(name='value_bias', shape=(self.att_embedding_size * self.head_num,),
                                       dtype=tf.float32,
                                       initializer=tf.keras.initializers.TruncatedNormal(seed=self.seed))
        self.position_emb = Embedding(self.seq_len_max, embedding_size, embeddings_initializer=tf.keras.initializers.TruncatedNormal(seed=self.seed))
    ...
    def call(self, inputs, mask=None, training=None, **kwargs):
        ...
        if self.use_positional_encoding:
            _, T, num_units = queries.get_shape().as_list()
            position_ind = tf.expand_dims(tf.range(T), 0)
            queries = queries + self.position_emb(position_ind)

            _, T, num_units = keys.get_shape().as_list()
            position_ind = tf.expand_dims(tf.range(T), 0)
            keys = keys + self.position_emb(position_ind)
    ...
    def compute_output_shape(self, input_shape):
        return (None, self.seq_len_query, self.att_embedding_size * self.head_num)
